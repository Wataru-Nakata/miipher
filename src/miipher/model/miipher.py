...
class Miipher(nn.Module):
    def __init__(self,n_phone_feature,n_speaker_embedding,n_ssl_feature,n_hidden_dim,n_conformer_blocks,n_iters) -> None:
        super().__init__()
        ...
        for i in range(n_conformer_blocks):
            self.conformer_blocks.append(
                FeatureCleanerBlock(n_hidden_dim,8)
            )
...
class FeatureCleanerBlock(nn.Module):
    def __init__(self,hidden_dim,num_heads) -> None:
        ...
        self.conformer_block = ConformerLayer(
            hidden_dim,
            hidden_dim*4,
            num_heads,
            31
